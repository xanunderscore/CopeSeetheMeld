...
public class Meld(Gearset goal, MeldOptions opts, MeldLog? log) : MeldCommon
{
    ...
    public static bool Satisfies(Mat actual, Mat desired) => actual.Id == desired.Id && actual.Grade >= desired.Grade;
    ...
    private async Task MeldOne(ItemRef foundItem, Mat m, int overmeldSlot = -1)
    {
        ...
        if (overmeldSlot >= 0)
        {
            await WaitWhile(() => !Game.IsAddonActive("SelectYesno"), "Yesno", 1);
            unsafe
            {
                Game.FireCallback("SelectYesno", [0], true);
            }
        }
